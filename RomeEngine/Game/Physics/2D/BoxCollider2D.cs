namespace RomeEngine
{
    public sealed class BoxCollider2D : Collider2D
	{
		...
		{
			if (collider is CircleCollider2D circle) return Intersection_Box_Circle(this, circle, data);
			else if (collider is BoxCollider2D box) return Intersection_Box_Box(this, box, data);
		...
		protected override bool RaycastCollider(Ray2D ray, out RaycastHit2D hit)
		{
			...
			bool c = new Rect(0f, 0f, 1f, 1f).IntersectsRay(new Ray2D(boxMatrix.MultiplyPoint(ray.origin), boxMatrix.MultiplyVector(ray.direction)), out float dist);
